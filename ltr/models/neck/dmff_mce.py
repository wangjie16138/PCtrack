...
class DMFF(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.conv1 = ConvReluBN(256, 512)
        self.conv2 = ConvReluBN(512, 256)
    ...
    def forward(self, x1, x2):
        fusion_features = []
        cat_feat = []
        for lvl, (sp_feat, tp_feat) in enumerate(zip(x1, x2)):
            fusion_features_one = torch.cat((sp_feat, tp_feat), dim=1)
            fusion_features.append(fusion_features_one)

        cat_feat.append(fusion_features[0])
        cat_feat_one = self.conv2(self.conv1(fusion_features[1]) + self.fuse1(fusion_features[0]))
        cat_feat.append(cat_feat_one)
        cat_feat_one = self.conv2(self.conv1(fusion_features[2]) + self.fuse1(cat_feat_one))
        cat_feat.append(cat_feat_one)
...
运动建模和细化
"""
class Motion_offset_correction(nn.Module):
    ...
    def make_final_layers(self, input_channel, layer_list):#(256,128,128,128,1)
        layers = []
    ...
    def forward(self, batch_dict):
        ...
        cat_feat = self.dmff(sp_feats, tp_feats)
        src_flatten = []
        spatial_shapes = []
        for feature in cat_feat:
            bs, c, x, y = feature.shape
            spatial_shape = (x, y)
        ...
        batch_dict['cls_preds'] = cls_preds.permute(0, 2, 3, 1).contiguous()
        batch_dict['fusion_feature'] = deform_sp_feats


        corner_off = batch_dict['predicted_box_corner_offset'][:, 0, :].reshape(BS, 3, 15)
        ...
        for itr in range(self.iters):
            ...
            net, delta_flow = self.MCE(net, inp, corr, flow)
