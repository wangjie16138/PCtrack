...
class CenterHeadTemplate(nn.Module):
    ...
    def assign_stack_targets(self, gt_boxes, real_height, search_points):
        ...
        reg_labels = gt_boxes.new_zeros((batch_size, self.max_num, 8))  # [batch_size, max_num, 8]
    ...
    def get_track_cls_loss_focal(self):
        cls_preds = self.forward_ret_dict['cls_preds']  # cls pred(batch_size, height, width, channel)
        cls_labels = self.forward_ret_dict['cls_labels']  # cls label
        batch_size, height, width, channel = cls_preds.shape 
        cls_preds = cls_preds.permute(0, 3, 1 ,2).contiguous()#(batch_size, channel, height, width)
        cls_labels = cls_labels.unsqueeze(1)# (batch_size, 1, height, width)
