...
from .Interactive_feature_extraction import IFE
...
class BaseBEVBackbone(nn.Module):
    def __init__(self, model_cfg, input_channels):
        ...
        self.IFE = IFE()
        c_in = self.create_layer(self.tp_blocks, self.tp_deblocks, num_levels, tp_c_in_list, layer_strides, num_filters,
                                 layer_nums, upsample_strides,
                                 num_upsample_filters)
        _ = self.create_layer(self.sr_blocks, self.sr_deblocks, num_levels, sr_c_in_list, layer_strides, num_filters,
                              layer_nums, upsample_strides,
                              num_upsample_filters)
    ...
    def create_layer(self, blocks, deblocks, num_levels, c_in_list, layer_strides, num_filters, layer_nums,
                     upsample_strides, num_upsample_filters):
    ...
    def forward(self, batch_dict):
        ...
        batch_dict['search_spatial_features_2d'], batch_dict['template_spatial_features_2d'] \
            = self.forward_feature(search_spatial_features, template_spatial_features, self.sr_blocks, self.sr_deblocks,
                                   self.tp_blocks, self.tp_deblocks)
    ...
    def forward_feature(self, sr_spatial_features, tp_spatial_features, sr_blocks, sr_deblocks, tp_blocks, tp_deblocks):
        ...
        sr_ups = self.IFE(sr_ups)
        tp_ups = self.IFE(tp_ups)
