...
class KittiSOTDataset(SOTDatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, eval_flag=False, root_path=None, logger=None):
        super().__init__(
            dataset_cfg=dataset_cfg, class_names=class_names, training=training, eval_flag=eval_flag,
    ...
    def image_show_with_box(self, image, info):
        cv2.rectangle(image, (int(info['bbox_left']), int(info['bbox_top'])),
                      (int(info['bbox_right']), int(info['bbox_bottom'])), (0, 0, 255), 2)
    ...
    def find_history_idx(self, index):
        pad_with_previous = self.history_frame_id_list  # 5
        template_list = [index + his_id for his_id in pad_with_previous]  # 当前帧加上偏移，获得历史帧索引
        first_frame = np.array(self.first_frame_index)  # 存储序列中每个场景的第一个帧索引
        seq_index = first_frame[first_frame <= index][-1]  # 确定当前帧 index 所属场景的第一个帧索引，用于限制索引范围
        template_list_re = [x if x >= seq_index else seq_index for x in
                            template_list]  # 如果候选帧索引 x 小于当前场景的第一个帧索引 seq_index，将其设置为 seq_index，确保历史帧索引不越界。
    ...
    # 网格化区域中标记了模版边界框的占用区域，用于后续特征提取和运动匹配，分辨率较低，适合大范围的粗略区域标定
    ...
    # ================================== generate history box ==================================
    def train_val_generate_future_box(self, search_box, history_box_list, local_template_box, future_box_list,
                                      local_search_box, random_ry):
        search_box_ = copy.deepcopy(search_box)
        local_search_box_ = copy.deepcopy(local_search_box)
    ...
    def get_train_tracking_item(self, index):
        ...
        # ---------- 获取两帧局部目标框，当前帧目标框有加强操作 ----------
        random_ry, local_search_points, center_offset, local_search_new_box, local_search_corner, local_search_area, \
            local_template_points, local_template_box_points, local_template_box = self.train_val_point_zone_func(
            search_box,
            template_box,
            search_points,
            template_points)
        ...
        # ---------- 获取当前帧目标框在BEV上掩码、运动掩码、运动偏移图、角度偏移图 ----------
        ...
        history_gt_box_lidar_list = np.vstack(list([one_box.get_lidar_box3d(search_calib).reshape(-1) for one_box in
                                                    history_gt_box_list]))  # search -> gt_box (lidar)
        # 生成历史帧的边界框序列，此时的历史边界框参数是相对当前帧和前一帧的得到的具有局部坐标系的关联的
        history_box = self.train_val_generate_history_box(search_boxes_lidar, history_gt_box_lidar_list,
                                                          local_search_new_box, local_template_box)
        local_search_new_box[2] = 0  # 设置目标高度为0，适合在BEV视角处理
        # 获取历史帧边界框的角点、中心点和旋转角信息，完善历史帧边界框信息，提供丰富的历史空间信息，用于时间序列建模
        history_box_corner = list([tracklet3d_kitti.Tracklet3d_Lidar(one_box).corners_and_face_centers() for one_box in
                                   history_box])  # 4个历史框的角点和面中心点（3，8+6）

        history_box_center = list([one_box[:3].reshape(-1, 1) for one_box in history_box])  # 一个中心点（3，1）

        history_box_ry = list([one_box[-1].reshape(-1, 1) for one_box in history_box])  # 一个角度（1，1）

        for ind in range(history_box.shape[0]):
            history_box_corner[ind] = np.concatenate((history_box_corner[ind], history_box_center[ind]),
                                                     axis=-1).reshape(1, -1)
            history_box_corner[ind] = np.concatenate((history_box_corner[ind], history_box_ry[ind]), axis=-1)

        local_template_box_corner = tracklet3d_kitti.Tracklet3d_Lidar(
            local_template_box).corners_and_face_centers()  # (3,14)

        local_template_box_corner = np.concatenate((local_template_box_corner, local_template_box[:3].reshape(-1, 1)),
                                                   axis=-1)  # (3,15)

        history_box_corner.insert(0, np.concatenate(
            (local_template_box_corner.reshape(1, -1), local_template_box[-1].reshape(-1, 1)), axis=-1))

        history_box_corner = np.vstack(history_box_corner)  # (5,3*(14+1)+1)

        local_search_box_corner = tracklet3d_kitti.Tracklet3d_Lidar(
            local_search_new_box).corners_and_face_centers()  # 3,14
        local_search_box_corner = np.concatenate((local_search_box_corner, local_search_new_box[:3].reshape(-1, 1)),
                                                 axis=-1)  # 3,15

        gt_box_corner_offset = local_search_box_corner - local_template_box_corner  # 获取真实偏移值#3,15

        gt_box_ry_offset = local_search_new_box[-1] - local_template_box[-1]  # 获取真实角度偏移量
        ...
        input_dict = {
            ...
            'gt_box_corner_offset': gt_box_corner_offset.reshape(1, -1),  # (1,45)
    ...
    def get_test_tracking_item(self, index):
        ...
        history_box_corner = list([tracklet3d_kitti.Tracklet3d_Lidar(one_box).corners_and_face_centers() for one_box in history_box])
    ...
    # ================== point zone ==================
    # 根据连续两帧目标边界框和点云数据生成局部连续两帧信息，包括增强的边界框等
