...
class Tracklet3d_Lidar(object):
    ...
    def corners(self, wlh_factor=1.0):
        ...
        return corners
    def corners_and_face_centers(self, wlh_factor=1.0):
        """
        Returns the bounding box corners and the center points of the six faces.
        :param wlh_factor: <float>. Multiply w, l, h by a factor to inflate or deflate the box.
        :return: <np.float: 3, 14>. First 8 columns are the corners, the last 6 columns are the face centers.
        """
        dx = self.dim[0] * wlh_factor
        dy = self.dim[1] * wlh_factor
        dz = self.dim[2] * wlh_factor
        ...
        # Translate corners (move them to the actual object position)
        x, y, z = self.center
        corners[0, :] = corners[0, :] + x
        corners[1, :] = corners[1, :] + y
        corners[2, :] = corners[2, :] + z
        ...
        face_centers = np.zeros((3, 6))  # (3, 6) to store the center of each face

        for i, indices in enumerate(face_indices):
            # Select the corner points for the current face
            selected_corners = corners[:, indices]

            # Calculate the center by averaging the corner coordinates
            face_centers[:, i] = np.mean(selected_corners, axis=1)

        # Concatenate corners and face centers
        all_features = np.hstack((corners, face_centers))
